...
const UserReturnProducts = () =>  {
	const [data, setData] = useState([]);
	const navigate = useNavigate();
	let userid =sessionStorage.getItem('userid').toString()
	
	useEffect(() => {		
		axios.get('http://localhost:5000/userreturnproducts?userid='+userid)
		  .then(res => {
	  ...
	  const handleClick = (id,productid,qty) => () => {
		console.log(id);		
		//useEffect(() => {
			axios.get('http://localhost:5000/userreturnproducts1?id='+id+"&userid="+userid+"&productid="+productid+"&qty="+qty)
			  .then(res => {
				//setData(res.data.data);
				console.log("Response : ", res.data)
				console.log("Id : ", id)
		...
		>
			...
			<table style={{width:'90%', left:'10px', position:'absolute'}}>
				...
				<tr style={{width:'800px'}}>
					...
					<th style={{width:'200px'}}>User Return Product</th>
				</tr>
				</thead>
				<tbody>
			...
			{data.map((row, index) => (
              <tr data-index={index}>  
				...
				<button onClick={handleClick(row.id,row.productid,row.Quantity)}>
      User Return Product
    </button> 
	</td>
...
export default UserReturnProducts;