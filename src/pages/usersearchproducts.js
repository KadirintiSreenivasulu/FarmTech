...
const UserSearchProducts = () => {
	...
	*/
	const handleClick = (id,ProductName, ProductType, qty, Price, Description) => () => {
		let quantity = parseInt(qty)
		if(quantity<=0){
			alert('Product is Out of Stock')
		}
		...
		sessionStorage.setItem('pname', ProductName);
		sessionStorage.setItem('ptype', ProductType);
		sessionStorage.setItem('qty', quantity);
		sessionStorage.setItem('price', Price);
		sessionStorage.setItem('desc', Description);
		let path = '/usersearchproducts1';
		navigate(path);
		}
	};
	useEffect(() => {
		axios.get('http://localhost:5000/usergetproducts')
			.then(res => {
				setData(res.data.data);
				console.log("Response : ", res.data)
					...
					<table style={{ width: '90%', left: '10px', position: 'absolute' }}>
						<thead>
							<tr style={{ width: '800px' }}>
								...
								<th style={{ width: '200px' }}>Buy Product</th>
							</tr>
						</thead>
						<tbody>
...
{data.map((row, index) => (
              <tr data-index={index}>  
				...
				<td style={{textAlign:'center'}}>{row.Description}</td>				
					...
					<td style={{ textAlign: 'center' }}>
											<button onClick={handleClick(row.id, row.ProductName, row.ProductType,  row.Quantity, row.Price, row.Description)}>
												Buy Product
											</button>
...
export default UserSearchProducts;